...
if ((godoNum == 1)); then
    echo 写3311行数$rows
    ~/go/bin/pump -d "MYSQL_PWD=root mysql -h127.0.0.1 -P3311 -uroot" -t aa.tr_f_db -r $rows --onerr=exit

elif ((godoNum == 2)); then
    echo 写3312行数$rows
    ~/go/bin/pump -d "MYSQL_PWD=root mysql -h127.0.0.1 -P3312 -uroot" -t aa.tr_f_db -r $rows --onerr=exit
